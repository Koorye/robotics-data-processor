...
def copy_annotation(src_parquet_path, dest_parquet_path):
    # e.g. lerobot_dataset/realman/raw_1/data/chunk-000/episode_000012.parquet
    # ->   lerobot_dataset/realman/raw_1/annotations/episode_000012.json
    src_path = src_parquet_path.replace("/data/", "/annotations/").replace(".parquet", ".json")
    dest_path = dest_parquet_path.replace("/data/", "/annotations/").replace(".parquet", ".json")
    episode_index = int(dest_path.split("_")[-1].split(".")[0])

    # remove chunk-xxx
    import re
    src_path = re.sub(r"/chunk-\d{3}/", "/", src_path)
    dest_path = re.sub(r"/chunk-\d{3}/", "/", dest_path)

    # 确保目标文件夹存在 (Ensure the target folder exists)
    ...
    with open(src_path) as f:
        ...
        with open(dest_path, "w") as f2:
            json.dump(annotation, f2, indent=4)
    ...
    for i, (old_folder, old_index, new_index) in enumerate(episode_mapping):
        ...
        if source_path:
            try:
                ...
                copy_annotation(source_path, dest_path)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--max_dim", type=int, default=34, help="Maximum dimension (default: 32)")
